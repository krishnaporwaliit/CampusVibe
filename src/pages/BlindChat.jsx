      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-3 pb-20 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-900/20 via-[#0F0A1A] to-[#0F0A1A]">
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[75%] p-3 rounded-2xl text-sm ${msg.sender === 'me' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white text-gray-900 rounded-bl-none'}`}>
              {msg.text}
            </div>
          </div>
        ))}
      ...
      <footer className="p-4 bg-white border-t border-indigo-500/20">
        <form onSubmit={handleSend} className="flex gap-2">
          <input 
            ...
            className="flex-1 bg-white border border-indigo-500/30 rounded-full px-4 py-2 text-sm text-gray-900 focus:outline-none focus:border-indigo-500 transition-colors"
          />
