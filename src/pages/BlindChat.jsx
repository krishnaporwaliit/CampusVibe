...
export default function BlindChat() {
  ...
  return (
    <div className="fixed inset-0 bg-[#0F0A1A] z-50 flex flex-col">
      <header className="bg-indigo-900/40 backdrop-blur-md p-4 flex items-center justify-between border-b border-indigo-500/20">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-sm text-white">{revealed ? "Alex Smith" : "Anonymous"}</h2>
        ...
        <button 
          ...
          className="text-xs bg-indigo-500 hover:bg-indigo-600 text-white px-3 py-1.5 rounded-full transition flex items-center gap-1"
        >
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-3 pb-20 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-900/20 via-[#0F0A1A] to-[#0F0A1A]">
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[75%] p-3 rounded-2xl text-sm ${msg.sender === 'me' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white/10 text-white rounded-bl-none'}`}>
              {msg.text}
            </div>
          </div>
        ))}
      ...
      <footer className="p-4 bg-[#1A1025] border-t border-indigo-500/20">
        <form onSubmit={handleSend} className="flex gap-2">
          <input 
            ...
            className="flex-1 bg-black/40 border border-indigo-500/30 rounded-full px-4 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors"
          />
          <button type="submit" className="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white hover:bg-indigo-700 transition">
